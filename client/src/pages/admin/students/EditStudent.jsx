...
export default function EditStudent() {
    ...
    try {
      ...
      if (student) {
        setFormData({
          ...
          year:        student.year        || '',
          warden1Id:   student.warden1Id?._id || '',
          warden2Id:   student.warden2Id?._id || '',
      ...
      setWarden1s(w1Res.data)
      setWarden2s(w2Res.data)
    } catch (err) {
      console.log(err)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatusMsg('Updating student info...')
      const payload = { ...formData }
      if (!payload.password) delete payload.password
      await editStudentApi(id, payload)

      const warnings = []

      // Step 2 — Update student face if new one captured
      if (studentFace) {
        setStatusMsg('Updating student face...')
        try {
          await registerStudentFaceApi(id, { image: studentFace })
        } catch (err) {
          console.log('Student face error:', err.response?.data)
          warnings.push('Student face update failed — try again later.')
        }
      }

      // Step 3 — Update parent faces if new ones uploaded
      for (let i = 0; i < parentFaces.length; i++) {
        if (parentFaces[i]) {
          setStatusMsg(`Updating ${formData.parents[i].relation} photo...`)
          try {
            await registerParentFaceApi(id, {
              image:       parentFaces[i],
              parentIndex: i
            })
          } catch (err) {
            ...
            warnings.push(
              `${formData.parents[i].relation} photo update failed — try again later.`
            )
          }
        }
      }
      ...
      if (warnings.length > 0) {
        ...
        setStatusMsg('⚠️ Student updated with some issues.')
        setTimeout(() => navigate('/admin/students'), 3000)
      } else {
        setStatusMsg('✅ Student updated successfully!')
    ...
    <div>
      ...
      <div style={styles.container}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.checkRow}>
              <input type='checkbox' name='isActive'
                ...
                onChange={handleChange} id='isActive' />
              <label htmlFor='isActive' style={styles.checkLabel}>
                Account Active
              </label>
            </div>
          </div>
          ...
          {/* Update Student Face — WEBCAM */}
          <div style={styles.card}>
            <h3 style={styles.cardTitle}>Update Student Face Photo</h3>
            <p style={styles.hint}>
              📸 Capture new face to update gate biometric scan
            </p>
            <WebcamCapture onCapture={(image) => setStudentFace(image)} />
            {studentFace && (
              <div style={styles.capturedRow}>
                <span style={styles.captured}>✅ New student face captured</span>
                <button
                  type='button'
          ...
          <div style={styles.card}>
            ...
            <p style={styles.hint}>
              📁 Update parent info and photos for face verification
            </p>
            ...
            {formData.parents.map((parent, i) => (
              <div key={i} style={styles.parentSection}>
                <h4 style={styles.parentTitle}>
                  {parent.relation}
                  {parentFaces[i] && (
                    <span style={styles.parentBadge}>✅ New photo ready</span>
                  )}
                </h4>
                ...
                <div style={styles.grid3}>
                  <div style={styles.field}>
                    ...
                    <input value={parent.name}
                      ...
                      style={styles.input} placeholder='Parent name' />
                  </div>
                  <div style={styles.field}>
                    <label style={styles.label}>Email</label>
                    <input type='email' value={parent.email}
                      onChange={e => handleParentChange(i, 'email', e.target.value)}
                      style={styles.input} placeholder='Parent email' />
                  </div>
                  <div style={styles.field}>
                    <label style={styles.label}>Phone</label>
                    <input value={parent.phone}
                      onChange={e => handleParentChange(i, 'phone', e.target.value)}
                      style={styles.input} placeholder='Parent phone' />
                  </div>
                </div>

                {/* FILE UPLOAD for parent photo */}
                <div style={styles.uploadSection}>
                  <label style={styles.label}>
                    📁 Upload New {parent.relation} Photo
                  </label>
                  ...
                  {!parentFaces[i] ? (
                    ...
                    <div style={styles.previewBox}>
                      ...
                      />
                      <div>
                        <p style={styles.captured}>
                          ✅ {parent.relation} photo ready
                        </p>
                        <button
                          type='button'
                          style={styles.clearBtn}
          ...
          <div style={styles.buttons}>
            ...
            <button type='submit' style={{
              ...
              {loading ? statusMsg || 'Saving...' : 'Save Changes'}
            </button>
          </div>
...
const styles = {
  ...
  checkRow:      { display: 'flex', alignItems: 'center', gap: '8px', marginTop: '16px' },
