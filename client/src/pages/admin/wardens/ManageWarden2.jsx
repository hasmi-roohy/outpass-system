...
export default function ManageWarden2() {
  ...
  const fetchWardens = async () => {
    try {
      ...
      setError(err.response?.data?.message || 'Failed to load Warden 2 list')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Unable to delete Warden 2')
      setDeleteId(null)
    } finally {
      setDeleting(false)
    }
    ...
    <div style={s.page}>
      ...
      <div style={s.container}>
        <div style={s.pageHeader}>
          <div>
            <h1 style={s.pageTitle}>Existing Warden 2s</h1>
            <p style={s.pageSub}>{wardens.length} gate wardens in the system</p>
          </div>
          <button
            style={s.addBtn}
              ...
              <div key={warden._id} style={s.card}>
                ...
                <div style={s.details}>
                  ...
                  <div style={s.detailItem}>
                    ...
                    <span style={{
                      ...s.assignedChip,
                      background: warden.assignedStudentCount > 0 ? '#f0fff4' : '#f5f5f5',
                      color: warden.assignedStudentCount > 0 ? '#059669' : '#888'
                    }}>
...
const s = {
  ...
  addBtn: { background: '#059669', color: '#fff', border: 'none', padding: '12px 24px', borderRadius: '10px', cursor: 'pointer', fontWeight: '700', fontSize: '14px' },
  ...
  emptyIcon: { width: '52px', height: '52px', margin: '0 auto 16px', borderRadius: '14px', background: '#f0fff4', color: '#059669', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '800' },
  ...
  avatar: { width: '48px', height: '48px', borderRadius: '50%', background: '#059669', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: '700', fontSize: '20px', flexShrink: 0 },
