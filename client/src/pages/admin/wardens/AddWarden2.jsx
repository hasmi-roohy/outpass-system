...
import { addWarden2Api } from '../../../api/api'

export default function AddWarden2() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await addWarden2Api(formData)
      navigate('/admin/warden2s')
    } catch (err) {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.container}>
        ...
        <button style={s.backBtn} onClick={() => navigate('/admin/warden2s')}>
          ← Back
        </button>

        <div style={s.pageHeader}>
          <h1 style={s.pageTitle}>Add Warden 2</h1>
          <p style={s.pageSub}>Add a new gate warden to the system</p>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={s.buttons}>
              <button
                type='button'
                style={s.cancelBtn}
                onClick={() => navigate('/admin/warden2s')}
                disabled={loading}
              >
                Cancel
              </button>
...
const s = {
  ...
  backBtn:   { background: 'none', border: 'none', color: '#059669', cursor: 'pointer', fontSize: '14px', fontWeight: '600', padding: '0 0 16px', display: 'block' },
  ...
  submitBtn: { background: 'linear-gradient(135deg, #059669, #047857)', color: '#fff', border: 'none', padding: '12px 28px', borderRadius: '10px', fontWeight: '700', fontSize: '14px' }
}